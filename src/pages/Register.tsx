...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.response?.data?.error || "Gagal membuat akun. Coba lagi.";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      <div className="hidden lg:flex flex-col justify-between p-12 text-white bg-gradient-to-br from-[#0B1F3A] via-[#13294e] to-emerald-900">
        ...
        <div>
          <h1 className="text-5xl font-black leading-tight">Bergabung<br />dengan DigiFin Quest.</h1>
          <p className="mt-4 text-slate-300 max-w-md text-lg">Buat akun dan mulai mengumpulkan poin, lencana, dan level hari ini.</p>
      ...
      <div className="flex items-center justify-center p-6">
        <Card className="w-full max-w-md p-8">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="text-sm text-slate-400">Register Mahasiswa</span>
            <ThemeToggle />
          </div>
          <h2 className="text-2xl font-bold mb-1">Buat akun Anda</h2>
          <p className="text-slate-500 dark:text-slate-400 mb-6 text-sm">Daftar sebagai mahasiswa untuk mulai belajar.</p>

          <form onSubmit={handleSubmit}>
            <label className="block text-sm font-medium mb-1">Nama Lengkap</label>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="mis. Rina Amelia"
